...
package final class JSCMacroRunner: WasmMacroRunner {
  package let memory: JSCGuestMemory
  package let api: JSValue

  package init(wasm data: Data, bridge: WASIBridgeToHost) async throws {
    ...
    let imports: [String: [String: JSValue]] = bridge.wasiHostModules.mapValues { module in
    ...
    self.memory = JSCGuestMemory(getMemory: getMemory, setMemory: setMemory)
    memory = self.memory
  }
...
package struct JSCGuestMemory: GuestMemory {
  let getMemory: JSValue
  let setMemory: JSValue

  package func withUnsafeMutableBufferPointer<T>(
    offset: UInt,
    count: Int,
